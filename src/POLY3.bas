3N%=RND(-TIME):PROCi:S%=0:REPEAT:MODE7:VDU23;8202;0;0;0;:HIMEM=&2BFF:PROCh:S%=0:l%=1:MODE2:VDU23;8202;0;0;0;:HIMEM=&2BFF:REPEAT:PROCo:PROCA:PROCa:PROCb:PROCn:IFl%>8:PROCf:PROCs:ELSEIF(l%MOD2)=1:PROCf:ELSE:PROCs
...
DEFPROCi:k%=&AB8:n%=&AC0:q%=&AC8:t%=&AD0:F%=&AD8:G%=&AE0:J%=&AE8:DIMh%(8),h$(8):RESTORE99:FORT%=1TO8:h%(T%)=(9-T%)*500:READh$(T%):NEXT:h%(1)=60000:ENDPROC
...
DEFPROCA:RESTORE(95+(l%MOD4)):READv:VDU28,0,4,19,0,17,v,12,26,5:READci%,m%:IFci%=1READw,x,y,z:PROCc(w,x,y,z)
...
cx%=FNg:VDU18,3,co%,25,4,cx%;192;cr%:ENDPROC
...
IFk%?E%>75q%?E%=1:F%?E%=2
IFn%?E%<64ANDq%?E%<>2THENt%?E%=8ELSEIFn%?E%<48ANDq%?E%=2THENn%?E%=48
...
DEFPROCp:SOUND3,0,0,1:FORz=1TO1000:NEXT:REPEAT:SOUND&11,-10,O%/8,2:VDU18,0,15,25,4,O%;52;25,5,O%;40;:O%=O%-8:S%=S%+50:PROCe:UNTILO%<920:O%=1224:ENDPROC
...
DEFPROCt:IFl%>8s%=94:ELSEIF(l%MOD2)=1:s%=92ELSEs%=93
RESTOREs%:d%=1:FORu=1TO7:READn:SOUNDd%,1,n,6:FORT=1TO400:NEXT:d%=d%+1:IFd%=4d%=1
NEXT:ENDPROC
...
UNTILI%=1ORh%(I%+(I%<>FALSE))>=S%:h%(I%)=S%:h$(I%)=STRING$(10,"."):ENDPROC
